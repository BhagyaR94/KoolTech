...
class GoodsReceivedController extends Controller {
    ...
    public function goodsreceived() {
        $last_rec = DB::table('tblt_goodsreceived')->select('Gr_No')->orderby('Gr_No', 'DESC')->first();
        $received = DB::table('tblt_goodsreceivedetail')->where('Gr_No','>',$last_rec->Gr_No)->get();
        return view('Frontend/InterfacesKT/Goods/GoodsReceived', ['last_rec' => $last_rec, 'received' => $received]);
    ...
    public function savereceived($code)
    {
        $net_amount=0;
        $gross_amount=0;
        $tot_dis=0;
        
        $detail=DB::table('tblt_goodsreceivedetail')->where('Gr_No',$code)->get();
    ...
    public function receivegood(Requests\Frontend\Goods\NewReceive $request) {
        $request->all();
        $grid = $request->ReceiveNo;
    ...
    public function cleareceive(Requests\Frontend\Goods\DropGoods $request) {
        ...
        $delete = DB::table('tblt_goodsreceivedetail')->where([
                    ['Gr_No', $grno],
                    ['Gr_LineNo', $line],
                ])->delete();
    ...
    public function savereceivedgoods(Requests\Frontend\Goods\SaveGoods $request)
    {
        $SNo=$request->SaveNo;
        $outcode = 1;
        $net_amount=0;
        ...
        $supplier;
        $userid = Auth::user()->id;
        $detail=DB::table('tblt_goodsreceivedetail')->where('Gr_No',$SNo)->get();
        foreach($detail as $det)
        {
            ...
            $net_amount=$gross_amount-$tot_dis;
            $stock=$det->Gr_Qty;
            DB::table('tblm_productdetail')->where('Pro_Code',$det->Gr_ProCode)->increment('Pro_Stock',number_format($stock, 2,'.',','));
        }
        $lastinvno=DB::table('tblt_invoice')->select('Inv_No')->orderby('Inv_No', 'DESC')->first();
        //$invno=$lastinvno+1;
        $insert = DB::insert('insert into tblt_goodsreceived (Gr_No,Gr_OutCode,Gr_Date,Gr_Time,Gr_SupCode,Gr_SupInvNo,Gr_Amount,Gr_UserCode) values (?,?,?,?,?,?,?,?) ', [$SNo, $outcode, \Carbon\Carbon::now('Asia/Colombo'), \Carbon\Carbon::now('Asia/Colombo'), $supplier, /*str_pad($invno, 8,'0',STR_PAD_LEFT)*/00000001, $net_amount,$userid]);
