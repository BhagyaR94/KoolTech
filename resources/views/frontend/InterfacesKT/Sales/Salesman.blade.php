...
<div class="box box-primary">
    <div class="box-header with-border">
        ...
        <div class="col-md-2">
            <h3 class="box-title"> Salesman </h3>
    ...
    <div class="box-body bg-gray-active">
        ...
        <div class="row-fluid">
            ...
            {!! Form::label ('qty_lbl','Salesman:',['class' =>'control-label col-md-1']) !!}
            <div class="col-md-5">
                {!! Form::text ('Salesman_No','',['class'=>'form-control input-sm', 'placeholder'=>'Salesman_No','autofocus']) !!}
            </div>
             </div>
            ...
            <table class="table table-borderless">
                ...
                <tr>
                    ...
                    <td>{{$invoice->Inv_GrossAmount}}</td>
                    <td>{{$invoice->Inv_ItemDiscount}}</td>
                    <td>{{$invoice->Inv_CusCode}}</td>
                    <td>{{$invoice->Inv_NetAmount}}</td>
                </tr>
                @endforeach
            </table>
            ...
            <table class="table table-borderless">
                ...
                @foreach($goodsrtn as $grn)
                <tr>
                <td>{{$grn->Gr_No}}</td>
                <td>{{$grn->Gr_Amount}}</td>
                <td>{{$grn->Gr_Date}}</td>
                <td>{{$grn->Gr_SupCode}}</td>
                </tr>
                @endforeach
            </table>
            {{$goodsrtn->links()}}
            
            <h3>Goods Receives</h3>
            <table class="table table-borderless">
                <thead>
                    <tr>
                        <th>Return_No</th>
                ...
                @foreach($goodsrcv as $grcv)
                <tr>
                <td>{{$grcv->Gr_No}}</td>
                <td>{{$grcv->Gr_Amount}}</td>
                <td>{{$grcv->Gr_Date}}</td>
                <td>{{$grcv->Gr_SupCode}}</td>
                </tr>
                @endforeach
            </table>
